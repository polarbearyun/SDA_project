						...
						<tbody>
							<c:forEach items="${page}" var="ord">
								<tr>
									...
									<td class="text-left"><c:forEach items="${ord.item}" var="item" varStatus="vs">
											<c:if test="${vs.index >0}"><br /></c:if>
											<a href="${ctx}/product_detail?id=${item.product_id}">${item.product_id}</a>${item.order_id} x ${item.amount}
				            </c:forEach></td>
									...
									<td class="text-center">下单时间：<fmt:formatDate
										...
										<%--<c:if test="${!empty ord.delivery_time}">--%>
											<%--<br />发货时间：--%>
										<%--<fmt:formatDate value="${ord.delivery_time}" pattern="yyyy-MM-dd HH:mm" />--%>
										<%--</c:if> --%>
									...
									<td class="text-center"><a
										href="${ctx}/user/orders/detail?id=${ord.id}">详情</a> <c:if
											test="${ord.status==4}" var="flag">
											<a href="${ctx}/user/orders/status?id=${ord.id}&status=5">确认收货</a>
										</c:if> <c:if test="${ord.status==3}" var="flag">
											<a href="${ctx}/user/orders/status?id=${ord.id}&status=-1">取消订单</a>
										</c:if>
								</tr>
							</c:forEach>
						</tbody>
